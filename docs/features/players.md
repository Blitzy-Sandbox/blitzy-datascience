...
Per-endpoint success and failure visibility is obtained from the canonical counters pre-registered in `utils/metrics.py` — Players does **not** define dedicated `pipeline_endpoint_*` counters (those are intentionally absent from the canonical catalog per AAP §0.7.3.1 and `docs/OBSERVABILITY.md`). On each attempt, `api/nba_client.py` increments `nba_requests_total{endpoint="<endpoint_name>"}`; on a permanent transport failure (tenacity retries exhausted), it increments `nba_request_failures_total{endpoint="<endpoint_name>", reason="<http_429|http_4xx_non_429|http_5xx|timeout|connection_error>"}`. Per-retry pressure is surfaced by `nba_retries_total{endpoint="<endpoint_name>"}`. On each successful CSV write, `pipelines/ingest_players.py` increments `pipeline_rows_written_total{pipeline="ingest_players", artifact="<players|player_tracking>"}` with the row count. Aggregate pipeline outcomes are recorded once per invocation as `pipeline_runs_total{pipeline="ingest_players", outcome="<success|error>"}`. All of these counters are exposed via `python run.py metrics` per [`../OBSERVABILITY.md`](../OBSERVABILITY.md). Per-endpoint success counts can be computed as `nba_requests_total - nba_request_failures_total` on a matching `{endpoint="..."}` selector.
...
- [`../OBSERVABILITY.md`](../OBSERVABILITY.md) — logging format, correlation ID mechanism, metrics catalog, health/readiness surface, dashboard template. Players-specific metrics:
  ...
  - `pipeline_runs_total{pipeline="ingest_players", outcome="success"}` / `{outcome="error"}` — incremented once per invocation to record the final disposition of the run.
  - `nba_requests_total{endpoint="<name>"}` — incremented per HTTP call inside `NBAClient.get`; filtering on Players endpoints (`leaguedashplayerstats`, `leaguedashplayerclutch`, `playercareerstats`, `playergamelog`, `leaguedashptstats`) yields the per-endpoint attempt count.
  - `nba_request_failures_total{endpoint="<name>", reason="<http_429|http_4xx_non_429|http_5xx|timeout|connection_error>"}` — incremented only when `tenacity` retries are exhausted on an endpoint; the per-endpoint success count is `nba_requests_total{endpoint="<name>"} - nba_request_failures_total{endpoint="<name>"}`.
